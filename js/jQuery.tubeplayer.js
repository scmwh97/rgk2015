...
(function($) {
			...
			new YT.Player(o.playerID, {
				...
				events: {
					...
					'onReady': function(evt) {
						...
						$("body").find(".videoMute").each(function(){ 
							try{ $(this).trigger("click"); } catch (e) { } 
						}); 
	...
	var PLAYER = {
		...
		unmute: wrap_fn(function(evt, param, p) {
			...
			var vol2 = p.$player.attr("data-prev-mute-volume") < 1 ? 50 : p.$player.attr("data-prev-mute-volume");
			p.ytplayer.setVolume((vol2 || 50));
		...
		destroy: wrap_fn(function(evt, param, p) {
			...
			$("body").find(".videoUnMute").each(function(){ 
				try{ $(this).trigger("click"); } catch (e) { } 
			});
